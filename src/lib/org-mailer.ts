...
export async function sendAsOrg(
  ...
  try {
    ...
    // No personal signature here: this path has no acting user (public/cron),
    // so whichever admin's token we borrowed would sign it arbitrarily. These
    // emails carry a business-level sign-off in their body instead.
