...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: invoice } = await supabase
    .from('invoices')
    .select('id, invoice_number, org_id, status, line_items, subtotal, tax, total, notes, deposit_credit, due_date, created_at, stripe_payment_link, contacts!invoices_contact_id_fkey(first_name, last_name, email, address_line1, suburb, state, postcode)')
    .eq('id', id)
