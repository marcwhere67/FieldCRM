...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from('notices').update({ ...body, updated_at: new Date().toISOString() }).eq('id', id).select('*, users!notices_created_by_fkey(full_name)').single()
